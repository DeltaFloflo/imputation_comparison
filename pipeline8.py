...
from build.utils import normalization, load_dataset, create_MARprob1bis
...
for i1 in range(NB_REPEAT_TRAIN):
    ...
    for i2 in range(len(LIST_DATASETS)):
        ...
        MARprob, idx_column = create_MARprob1bis(dataset_name)
        for c in range(original_data.shape[1]):
            if c==idx_column:
                print(f"MAR idx_column={idx_column}")
            else:
                miss_mask[:, c] = (r[:, c] < MARprob)  # An example of MAR scenario
        ...
        print("(" + strftime("%Y-%m-%d %H:%M:%S", localtime()) + ")", end=" ", flush=True)
...
if not os.path.exists("results/pipeline8/"):
    os.makedirs("results/pipeline8/")
if not os.path.exists("results/pipeline8/plots"):
    os.makedirs("results/pipeline8/plots")

np.save("results/pipeline8/rmse_gain.npy", rmse_gain)
np.save("results/pipeline8/rmse_misgan.npy", rmse_misgan)
np.save("results/pipeline8/rmse_knn1.npy", rmse_knn1)
np.save("results/pipeline8/rmse_knn2.npy", rmse_knn2)
...
plt.ylabel("RMSE")
plt.title("[MAR setting] mean missing_rate of 20%")
plt.legend()
plt.tight_layout()
plt.savefig("results/pipeline8/plots/rmse.pdf")
...
plt.legend()
plt.tight_layout()
plt.savefig("results/pipeline8/plots/rmse2.pdf")
...
plt.legend()
plt.tight_layout()
plt.savefig("results/pipeline8/plots/rmse3.pdf")
