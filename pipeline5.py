...
LIST_DATASETS = ["breast", "credit", "letter", "news", "spam", "wine_red", "wine_white"]
NB_NEIGHBOURS = 50  # For 1000 rows
NB_EPOCHS_GAIN = 20000  # For 1000 rows
NB_EPOCHS_MISGAN = 5000  # For 1000 rows
NB_REPEAT_TRAIN = 20
...
for i1 in range(NB_REPEAT_TRAIN):
    ...
    for i2 in range(len(LIST_DATASETS)):
        ...
        myGAIN = GAIN(dim=original_data.shape[1])  # For GAIN and MisGAN
        ...
        print(f"Train GAIN for {cur_gain_epochs} epochs...")
        myGAIN.train(norm_data, batch_size=128, epochs=cur_gain_epochs)
        print(f"Train MisGAN for {cur_misgan_epochs} epochs...")
        ...
        print(f"KNN uses {cur_knn_neighbours} neighbours")
...
if not os.path.exists("results/pipeline5/"):
    os.makedirs("results/pipeline5/")
if not os.path.exists("results/pipeline5/plots"):
    os.makedirs("results/pipeline5/plots")

np.save("results/pipeline5/rmse_gain.npy", rmse_gain)
np.save("results/pipeline5/rmse_misgan.npy", rmse_misgan)
np.save("results/pipeline5/rmse_knn1.npy", rmse_knn1)
np.save("results/pipeline5/rmse_knn2.npy", rmse_knn2)
...
x_ticks_locs = np.arange(0.0, 7.0)
...
plt.ylabel("RMSE")
plt.title("Missing rate 20% (MCAR)")
plt.legend()
plt.tight_layout()
plt.savefig("results/pipeline5/plots/rmse_datasets1.pdf")


# Plot 2: Remove the bad performances for the dataset News
mu1 = np.mean(rmse_gain, axis=0)[[0, 1, 2, 4, 5, 6]]
std1 = np.std(rmse_gain, axis=0)[[0, 1, 2, 4, 5, 6]]
mu2 = np.mean(rmse_misgan, axis=0)[[0, 1, 2, 4, 5, 6]]
std2 = np.std(rmse_misgan, axis=0)[[0, 1, 2, 4, 5, 6]]
mu3 = np.mean(rmse_knn1, axis=0)[[0, 1, 2, 4, 5, 6]]
std3 = np.std(rmse_knn1, axis=0)[[0, 1, 2, 4, 5, 6]]
mu4 = np.mean(rmse_knn2, axis=0)[[0, 1, 2, 4, 5, 6]]
std4 = np.std(rmse_knn2, axis=0)[[0, 1, 2, 4, 5, 6]]
x_ticks_locs = np.arange(0.0, 6.0)
x_ticks_names = ["breast", "credit", "letter", "spam", "wine_red", "wine_white"]
plt.figure(figsize=(8, 4))
plt.errorbar(x_ticks_locs-0.15, mu1, yerr=std1, color="indigo", marker=".", capsize=5.0, ls="", label="GAIN")
plt.errorbar(x_ticks_locs-0.05, mu2, yerr=std2, color="grey", marker=".", capsize=5.0, ls="", label="MisGAN")
plt.errorbar(x_ticks_locs+0.05, mu3, yerr=std3, color="teal", marker=".", capsize=5.0, ls="", label="KNN-uniform")
...
plt.legend()
plt.tight_layout()
plt.savefig("results/pipeline5/plots/rmse_datasets2.pdf")


# Plot 3: Remove the performances of MisGAN (only GAIN vs KNN)
mu1 = np.mean(rmse_gain, axis=0)[[0, 1, 2, 4, 5, 6]]
std1 = np.std(rmse_gain, axis=0)[[0, 1, 2, 4, 5, 6]]
mu3 = np.mean(rmse_knn1, axis=0)[[0, 1, 2, 4, 5, 6]]
std3 = np.std(rmse_knn1, axis=0)[[0, 1, 2, 4, 5, 6]]
...
plt.legend()
plt.tight_layout()
plt.savefig("results/pipeline5/plots/rmse_datasets3.pdf")